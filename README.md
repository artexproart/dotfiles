...
cd ~/dotfiles
stow bash git alacritty nvim xsession obsidian yandex-music
...
## Интеграция Perplexity AI как Chrome App

- Веб-приложение Perplexity AI установлено как отдельное приложение Google Chrome с помощью опции "Открывать в отдельном окне".
- Файл .desktop для Perplexity AI хранится в dotfiles в пакете `apps` и управляется через GNU Stow.

### Как использовать

1. Запустите Perplexity AI через Rofi или меню приложений.
2. Окно автоматически откроется.

### Файлы и конфигурация

- `apps/.local/share/applications/chrome-kpmdbogdmbfckbgdfdffkleoleokbhod-Default.desktop` — файл запуска приложения.
